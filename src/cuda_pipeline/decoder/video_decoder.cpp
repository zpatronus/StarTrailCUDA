...
VideoDecoder::VideoDecoder(const std::string& input_path, int frame_step,
    ...
    : input_path_(input_path), frame_step_(frame_step), output_queue_(output_queue),
      fmt_ctx_(nullptr), codec_ctx_(nullptr), hw_device_ctx_(nullptr), video_stream_idx_(-1),
      width_(0), height_(0), fps_(30), total_frames_(0), running_(false),
...
void VideoDecoder::start() {
    ...
    if (!hw_decoder_name) {
        std::cerr << "ERROR: No hardware decoder available for this codec.\n"
                  << "This tool requires NVENC/NVDEC hardware support.\n"
                  << "Supported codecs: H.264, HEVC, VP9, AV1\n"
                  << "Please use the baseline version for software decoding." << std::endl;
    ...
    if (avcodec_parameters_to_context(test_ctx, video_stream->codecpar) < 0) {
        std::cerr << "ERROR: Failed to copy codec parameters for hardware decoder." << std::endl;
        avcodec_free_context(&test_ctx);
        throw std::runtime_error("Hardware decoder initialization failed");
    }

    if (avcodec_open2(test_ctx, decoder, nullptr) < 0) {
        std::cerr << "ERROR: Hardware decoder " << hw_decoder_name << " not supported.\n"
                  << "This tool requires NVENC/NVDEC hardware support.\n"
                  << "Please use the baseline version for software decoding." << std::endl;
        avcodec_free_context(&test_ctx);
        throw std::runtime_error("Hardware decoder not supported");
    }
    ...
    std::cout << "Created CUDA hardware device context for GPU-direct decoding" << std::endl;
        ...
        if (avcodec_send_packet(codec_ctx_, packet) >= 0) {
            while (avcodec_receive_frame(codec_ctx_, decoded_frame) >= 0) {
                ...
                if (should_process) {
                    ...
                    CUDA_CHECK(cudaMemcpy2D(frame.d_uv_data, frame.uv_pitch,
                                            src_nv12 + src_pitch * height_, src_pitch, width_,
                                            uv_height, cudaMemcpyDeviceToDevice));
