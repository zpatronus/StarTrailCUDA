...
VideoEncoder::VideoEncoder(const std::string& output_path, int width, int height, int fps,
    ...
    : output_path_(output_path), width_(width), height_(height), fps_(fps),
      input_queue_(input_queue), fmt_ctx_(nullptr), codec_ctx_(nullptr), av_stream_(nullptr),
      av_frame_(nullptr), hw_device_ctx_(nullptr), hw_frames_ctx_(nullptr), running_(false),
      frame_count_(0), write_running_(false) {
...
void VideoEncoder::start() {
    ...
    std::cout << "Created CUDA hardware device context for GPU-direct encoding" << std::endl;
...
void VideoEncoder::encode_loop() {
    ...
    while (running_) {
        ...
        CUDA_CHECK(cudaMemcpy2D(dst_nv12, dst_pitch, input_frame.d_y_data, input_frame.y_pitch,
                                width_, height_, cudaMemcpyDeviceToDevice));

        CUDA_CHECK(cudaMemcpy2D(dst_nv12 + dst_pitch * height_, dst_pitch, input_frame.d_uv_data,
                                input_frame.uv_pitch, width_, height_ / 2,
        ...
        if (avcodec_send_frame(codec_ctx_, av_frame_) >= 0) {
            while (true) {
                ...
                if (ret >= 0) {
                    ...
                    total_encode_time_us_ += std::chrono::duration_cast<std::chrono::microseconds>(
